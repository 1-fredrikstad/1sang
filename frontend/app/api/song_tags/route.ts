...
export async function GET(req: Request) {
  try {
    ...
    const target = `${supabaseUrl}/rest/v1/song_tags?select=*${limit ? `&limit=${limit}` : ''}`;
...
export async function POST(req: Request) {
  try {
    const { supabaseUrl, anonKey } = getEnv();
    const { song_id, tag_id } = await req.json().catch(() => ({}));

    if (!song_id || !tag_id) {
      return NextResponse.json({ ok: false, error: 'song_id and url required' }, { status: 400 });
    }

    const res = await fetch(`${supabaseUrl}/rest/v1/song_tags`, {
      method: 'POST',
      headers: {
        apikey: anonKey,
        Authorization: `Bearer ${anonKey}`,
...
export async function DELETE(req: Request) {
  try {
    ...
    if (!song_id || !tag_id) {
      return NextResponse.json(
        { ok: false, error: 'song_id and tag_id required' },
        { status: 400 }
      );
    }

    const target =
      `${supabaseUrl}/rest/v1/song_tags` +
      `?song_id=eq.${encodeURIComponent(song_id)}` +
      `&tag_id=eq.${encodeURIComponent(tag_id)}`;
